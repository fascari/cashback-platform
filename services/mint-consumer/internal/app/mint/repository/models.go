...
func (mintRequestModel) TableName() string { return "mint.mint_requests" }

func (m mintRequestModel) toDomain() domain.MintRequest {
	return domain.MintRequest{
		ID: m.ID, CashbackID: m.CashbackID, UserID: m.UserID,
		WalletAddress: m.WalletAddress, TokenAmount: m.TokenAmount,
		IdempotencyKey: m.IdempotencyKey, Status: m.Status,
		RetryCount: m.RetryCount, MaxRetries: m.MaxRetries,
		TransactionHash: m.TransactionHash, BlockNumber: m.BlockNumber,
		ErrorCode: m.ErrorCode, ErrorMessage: m.ErrorMessage,
		NextRetryAt: m.NextRetryAt, CreatedAt: m.CreatedAt,
		UpdatedAt: m.UpdatedAt, CompletedAt: m.CompletedAt,
	}
}

func mintRequestFromDomain(r domain.MintRequest) mintRequestModel {
	return mintRequestModel{
		ID: r.ID, CashbackID: r.CashbackID, UserID: r.UserID,
		WalletAddress: r.WalletAddress, TokenAmount: r.TokenAmount,
		IdempotencyKey: r.IdempotencyKey, Status: r.Status,
		RetryCount: r.RetryCount, MaxRetries: r.MaxRetries,
		TransactionHash: r.TransactionHash, BlockNumber: r.BlockNumber,
		ErrorCode: r.ErrorCode, ErrorMessage: r.ErrorMessage,
		NextRetryAt: r.NextRetryAt, CompletedAt: r.CompletedAt,
	}
}
...
func (processedEventModel) TableName() string { return "mint.processed_events" }

func (m processedEventModel) toDomain() domain.ProcessedEvent {
	return domain.ProcessedEvent{ID: m.ID, EventID: m.EventID, EventType: m.EventType, CreatedAt: m.CreatedAt}
}

func processedEventFromDomain(e domain.ProcessedEvent) processedEventModel {
	return processedEventModel{ID: e.ID, EventID: e.EventID, EventType: e.EventType}
}
