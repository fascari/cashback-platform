...
func TestHandle_ShouldCallUseCaseWithParsedPayload(t *testing.T) {
	...
	repo.EXPECT().CreateMintRequestIdempotent(mock.Anything, mock.Anything, mock.Anything, mock.Anything).
		Return(domain.MintRequest{ID: 42}, true, nil)
	...
	uc := mintcashback.NewUseCase(repo, blockchain)
