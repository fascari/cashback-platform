...
// BlockReader retrieves the latest confirmed block number from the chain.
type (
	...
	// It calls the provided DepositHandler for every new event detected.
	// Restart recovery is handled by seeding startBlock from the caller (see bootstrap/deposit.go).
...
// WithPollInterval overrides the default poll interval. Used in tests and benchmarks.
...
// seedBlock returns a safe starting block when no prior state is available.
