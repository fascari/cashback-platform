...
func TestMintToken_ShouldReturnLockUnavailableWhenNonceLockNotAcquired(t *testing.T) {
	...
	nonceRepo.EXPECT().CurrentNonce(mock.Anything, testdata.SignerAddress).Return(int64(1), nil)
	nonceRepo.EXPECT().Increment(mock.Anything, testdata.SignerAddress).Return(int64(0), errors.New("lock held"))
...
func TestMintToken_ShouldSyncNonceAndMarkFailedWhenSendFails(t *testing.T) {
	...
	txRepo.EXPECT().FindByIdempotencyKey(mock.Anything, key).Return(nil, nil)
	nonceRepo.EXPECT().CurrentNonce(mock.Anything, testdata.SignerAddress).Return(int64(1), nil)
	nonceRepo.EXPECT().Increment(mock.Anything, testdata.SignerAddress).Return(testdata.Nonce, nil)
	...
	nonceRepo.EXPECT().SyncFromChain(mock.Anything, testdata.SignerAddress, testdata.OnChainNonce).Return(nil)
